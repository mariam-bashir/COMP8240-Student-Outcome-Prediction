...
# loading the prepared UCI completed-student dataset
try:
    data_existing = pd.read_csv(
        "data/processed/existing_dataset/completed_students.csv"
    )
...
# Clean column names
data_existing.columns = data_existing.columns.str.strip()


# Categorical predictors identified from the UCI dataset metadata
categorical_cols = [
    "Marital status",
    "Application mode",
    "Course",
...
X = data_existing.drop(columns=["Outcome"])


# One-hot encode categorical predictors
# drop_first=True follows the encoding style used in Farhood's script
X = pd.get_dummies(
    X,
...
print("Dataset shape:", data_existing.shape)
print("Encoded predictor count:", X.shape[1])
print("Target distribution:")
print(Y.value_counts().rename(index={0: "Failure", 1: "Success"}))
...
# Validation checks
assert data_existing.shape == (3630, 37)
assert X.shape[0] == 3630
assert X.shape[1] == 229
assert (Y == 1).sum() == 2209
assert (Y == 0).sum() == 1421
assert X.isnull().sum().sum() == 0

print("Existing-dataset preprocessing validation: PASSED")
...
plt.tight_layout()
plt.savefig("figures/existing_dataset/holdout/with_lasso_boxplot.png", dpi=300, bbox_inches="tight")
plt.show()
...
plt.xticks(rotation=45)
plt.tight_layout()
plt.savefig("figures/existing_dataset/holdout/without_lasso_boxplot.png", dpi=300, bbox_inches="tight")
plt.show()

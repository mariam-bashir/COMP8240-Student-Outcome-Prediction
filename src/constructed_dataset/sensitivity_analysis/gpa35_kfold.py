...
# source_dataset is retained only for traceability
# and must not be used as a model predictor
data_math = data_math.drop(columns='source_dataset')


# Convert the constructed Higher/Lower outcome into the same
# Pass/Fail-style target structure used in Farhood's implementation
data_math['Result'] = [
...
non_numerical_cols = data_math.select_dtypes(include=['object', 'string']).columns
...
plt.savefig(FIGURE_DIR / "with_lasso_boxplot.png", dpi=300, bbox_inches="tight")
...
plt.savefig(FIGURE_DIR / "without_lasso_boxplot.png", dpi=300, bbox_inches="tight")

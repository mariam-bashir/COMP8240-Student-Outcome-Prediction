...
# =========================================================
# Dataset-specific setup for the constructed Part 3 dataset
# =========================================================
...
# Loading the constructed dataset
try:
    data_math = pd.read_csv(DATA_PATH)
except Exception as e:
    print("Reading the CSV file resulted in an error:")
    print(str(e))
    raise

# source_dataset is retained for traceability only and is not a predictor
data_math = data_math.drop(columns="source_dataset")

# Convert the Part 3 outcome to Farhood's Pass/Fail target structure.
# Higher -> Pass and Lower -> Fail. This keeps the downstream Farhood
# modelling code using Result_Pass unchanged.
data_math["Result"] = [
    "Pass" if x == "Higher" else "Fail"
    for x in data_math["outcome"]
]
data_math = data_math.drop(columns="outcome")

# Printing the number of columns, column names, and first few rows
print(data_math.columns)
print(data_math.head())
print(data_math.shape[1])

# Identify columns that are not numeric
non_numerical_cols = data_math.select_dtypes(
    include=["object", "string"]
).columns

# Encoding non-numerical columns with one-hot encoding
# Kept consistent with Farhood's implementation.
data_mathf = pd.get_dummies(
    data_math,
    columns=non_numerical_cols,
    drop_first=True,
)
...
# Kept in the same location as Farhood's original implementation.
scaler = StandardScaler()
X_scaled = scaler.fit_transform(X)


# =========================================================
# Farhood holdout modelling/evaluation logic
# =========================================================

# Machine learning models by Holdout
...
accuracy_valuesXGB = []  # Reserved space for XGBoost accuracy values
accuracy_valuesXGB_lasso = []  # Reserved space for XGBoost with Lasso accuracy values
...
accuracy_values_with_lasso = [
    accuracy_valuesLiR_lasso,
    accuracy_valuesLoR_lasso,
    accuracy_valuesSVM_lasso,
    accuracy_valuesDT_lasso,
    accuracy_valuesRF_lasso,
    accuracy_valuesKNN_lasso,
    accuracy_valuesXGB_lasso,
]
...
plt.tight_layout()
plt.savefig(
    FIGURE_DIR / "with_lasso_boxplot.png",
    dpi=300,
    bbox_inches="tight",
)
plt.show()
...
plt.boxplot(
    accuracy_values_without_lasso,
    tick_labels=labels_without_lasso,
    vert=False,
    showmeans=True,
)
plt.xlabel("Accuracy")
...
plt.tight_layout()
plt.savefig(
    FIGURE_DIR / "without_lasso_boxplot.png",
    dpi=300,
    bbox_inches="tight",
)
plt.show()

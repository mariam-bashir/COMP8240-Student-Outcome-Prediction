...
# loading the xAPI-Edu-Data dataset
try:
    data_math = pd.read_csv("data/raw/reproduction/xAPI-Edu-Data.csv")
...
# Retain the four most frequent categories and map the rest to Other
for col in ["NationalITy", "PlaceofBirth", "Topic"]:
    top4 = data_math[col].value_counts().nlargest(4).index
    data_math[col] = data_math[col].where(
        data_math[col].isin(top4),
        "Other"
    )


# Convert GradeID to its ordinal numeric value
data_math["GradeID"] = (
    data_math["GradeID"]
...
# Farhood et al.: Low = 1, Middle = 2, High = 3
class_numeric = data_math["Class"].map({
    "L": 1,
    "M": 2,
    "H": 3
})
...
# Middle and High = Pass; Low = Fail
Y = (class_numeric > 1).astype(int)


# Remove target column from predictors
X = data_math.drop(columns=["Class"])
...
# Retain the original variable name for consistency with Farhood's script
data_mathf = X


# Validation checks
print("Dataset shape:", data_math.shape)
print("Predictor count:", X.shape[1])
print("Target distribution:")
print(Y.value_counts().rename(index={0: "Fail", 1: "Pass"}))
print(X.head())
...
plt.tight_layout()
plt.savefig("figures/reproduction/xapi/holdout/with_lasso_boxplot.png", dpi=300, bbox_inches="tight")
plt.show()
...
plt.xticks(rotation=45)
plt.tight_layout()
plt.savefig("figures/reproduction/xapi/holdout/without_lasso_boxplot.png", dpi=300, bbox_inches="tight")
plt.show()
